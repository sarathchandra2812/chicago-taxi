...
def initialize_params():
    ...
    args_parser.add_argument(
        '--eval_file_name',
        help='Specifying the file name for storing the file for evaluation.',
        default='eval.csv' 
    )
    args_parser.add_argument(
        '--eval_bq',
        help='The BigQuery Table for the eval data',
...
def main():
    ...
    eval_sql = generate_sql(parameters.eval_bq)
    eval1 = get_raw(client, eval_sql)
    ...
    by_driver = get_gcs(drivers_gcs_path)
    eval2 = merge_driver_cluster(eval1, by_driver)
    ...
    eval2.k2.fillna(popular_k2, inplace = True)
    eval2.k2 = eval2.k2.astype(np.int64)
    
    eval3 = is_luxury(eval2)
    eval4 = filter_data(eval3, parameters.distance_upper_limit, parameters.mph_upper_limit)
    eval5 = eval4[['hour','weekday','pickup_latitude','pickup_longitude','dropoff_latitude','dropoff_longitude', 'k2','is_luxury','fare_dollars']]

    eval_gcs_path = 'gs://'+ parameters.bucket_name + '/data/csv/' + parameters.eval_file_name
    save_gcs(eval5, eval_gcs_path)
    print ('Eval file sent to Google Cloud Storage.' + eval_gcs_path)
